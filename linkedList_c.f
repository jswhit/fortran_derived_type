...
subroutine add_(self,key,ptr)
 class(registry_t) :: self
 integer,intent(out)   :: key
 type (LISTED_TYPE), intent(in), pointer :: ptr
 ...
 !allocate next element and assign key and data
 allocate(next)
 next%key = key
 next%element = ptr
...
subroutine get_(self,key,ptr)
 ...
 type (LISTED_TYPE), intent(out), pointer :: ptr
